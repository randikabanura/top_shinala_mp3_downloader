...
class SongREG:
    ...
    def store(self, arr):
        ...
        if row_count == 0:
            c.execute(f'INSERT INTO song(source_id, name, path, song_desc, status, genre_idgenre) VALUES({arr["source_id"]}, "{arr["song_name"]}", "{arr["path"]}", "{arr["song_desc"]}", 0, 1)')
            self.conn.commit()
            ...
            if row_count == 1:
                ...
                if row_count != 0:
                    ...
                    if row_count == 0:
                        ...
                        c.execute(f'INSERT INTO artist_name(name, artist_idartist) VALUES("{arr["artist_name"]}", {artist_id})')
                        self.conn.commit()
                        c.execute(f'INSERT INTO song_has_artist(song_idsong, artist_idartist) VALUES({song_id}, {artist_id})')
                        self.conn.commit() 
                ...
                if is_new:
                    ...
                    if row_count == 0:
                        ...
                        c.execute(f'INSERT INTO artist_name(name, artist_idartist) VALUES("{arr["artist_name"]}", {latest_artist_id})')
                        self.conn.commit()
                        c.execute(f'INSERT INTO song_has_artist(song_idsong, artist_idartist) VALUES({song_id}, {latest_artist_id})')
                        self.conn.commit()
